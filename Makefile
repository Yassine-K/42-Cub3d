...
BONUS_OBJ = $(addprefix $(SRC_DIR), $(SRC))


NAME = cub3D
CFLAGS= -O3 -Ofast -flto   -Wall -Wextra -Werror # -g -fsanitize=thread
...
$(NAME) : ascci_art $(OBJ)
	...
	@printf "$(RED)------------------------------------------ Cub3d Done -------------------------------------------$(RESET)\n"

bonus : fclean ascci_art
	@make -C libft > /dev/null
	@$(CC)  $(BONUS_OBJ) -D BONUS=1 $(CFLAGS) $(MLX_FLAGS) $(INCLUDE) libft/libft.a  -o cub3D_bonus
	@printf "$(RED)------------------------------------------ Cub3d_bonus Done -------------------------------------------$(RESET)\n"
...
fclean:
	...
	@rm -rf $(OBJ_DIR) $(NAME) cub3D_bonus
