...
{
	if (BONUS && mlx_is_key_down(vars->mlx, MLX_KEY_SPACE))
		enemy_shooting(vars);
	if (BONUS && mlx_is_key_down(vars->mlx, MLX_KEY_1))
		vars->hud->selected_gun = 0;
	if (BONUS && mlx_is_key_down(vars->mlx, MLX_KEY_2))
		vars->hud->selected_gun = 1;
	if (BONUS && mlx_is_key_down(vars->mlx, MLX_KEY_3))
		if (vars->hud->slots[2])
			vars->hud->selected_gun = 2;
	if (BONUS && mlx_is_key_down(vars->mlx, MLX_KEY_4))
		if (vars->hud->slots[3])
